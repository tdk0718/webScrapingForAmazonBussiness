      ...
      for (let t = 0; t < lines.length; t++) {
        ...
        try {
          await ItemRef.doc(recordData.asin).update(
            __spreadValues(__spreadValues({}, recordData), { update_at: new Date() })
          )
        } catch (e) {}
      }
      return resolve('ok')
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      while (res[0].path.indexOf('.crdownload') !== -1) {
        ...
        res = files.length
          ? (0, import_fast_sort.sort)(files).desc(e => e.sortNum)
          : [{ path: '.crdownload' }]
        console.log(res)
      }
    ...
    try {
      ...
      while (itemsData.getDocs(9999).length) {
        ...
        for (let i = 0; i <= itemsData.getDocs(9999).length; i++) {
          if ((_a = itemsData.getDocs(9999)[i]) == null ? void 0 : _a.asin) {
            await typeTextByCss(
              driver2,
              '#importInputAsin',
              ((_b = itemsData.getDocs(9999)[i]) == null ? void 0 : _b.asin) + ' '
            )
        ...
        const df = is_mac2
          ...
          : 'C:\xA5Users\xA5Administrator\xA5Downloads'
