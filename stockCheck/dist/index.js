...
var logsData = {
  ...
  getLatestDoc() {
    ...
    const maxSearchIndex = (0, import_fast_sort.sort)(result).desc(r => r.searchTextIndex)[0]
      .searchTextIndex
    const maxNodeIndex = (0, import_fast_sort.sort)(
      result.filter(e => e.searchTextIndex === maxSearchIndex)
    ).desc(r => r.nodeIndex)[0].nodeIndex
    const maxPageNum = (0, import_fast_sort.sort)(
...
var categories = [
  ...
  {
    code: 171350011,
    keyword: '\u53CC\u773C\u93E1\u30FB\u671B\u9060\u93E1\u30FB\u5149\u5B66\u6A5F\u5668',
  },
  {
    code: 128187011,
    keyword: '\u643A\u5E2F\u96FB\u8A71\u30FB\u30B9\u30DE\u30FC\u30C8\u30D5\u30A9\u30F3',
  },
...
async function getAmazonInfo() {
  ...
  try {
    ...
    const latestLogDate = ((_a = logsDataObj == null ? void 0 : logsDataObj.created_at) == null
    ? void 0
    : _a.seconds)
      ? new Date(logsDataObj.created_at.seconds * 1e3)
      : new Date(0)
    const now = new Date()
    let checkLogData = {}
    if (
    ...
    for (
      ...
      for (
        ...
        while (pageNum < 1e3) {
          ...
          const pageOverFlowArray =
            (_b = pageOverFlow == null ? void 0 : pageOverFlow.replace(' \u4EE5\u4E0A', '')) == null
              ? void 0
              : _b.split(' ')
          ...
          for (let i = 1; i <= numPerPage.length; i++) {
            ...
            const el = await driver[n].findElements(
              By2.css('.s-result-item.s-asin:nth-child(' + i + ') h2.a-color-base')
            )
            console.log(i)
            const today = new Date()
            if (el.length) {
              const asin = await driver[n]
                .findElement(By2.css('.s-result-item.s-asin:nth-child(' + i + ')'))
                .getAttribute('data-asin')
              const priceExist = await driver[n].findElements(
                By2.css('.s-result-item.s-asin:nth-child(' + i + ') span.a-price-whole')
              )
              if (priceExist.length) {
                ...
                if (
                  Number(priceInJp) > 3500 &&
                  !((_c = itemsData.getDocById(asin)) == null ? void 0 : _c.id)
                ) {
                  await driverInKeepa.get('https://keepa.com/#!product/1-' + asin)
                  try {
                    await driverInKeepa.wait(
                      until2.elementLocated(By2.css('span.priceNewsss')),
                      1e3
                    )
                  } catch (e) {}
                  try {
                  ...
                  const amazonPriceInUSNumber = await driverInKeepa.findElements(
                    By2.css('span.priceAmazon')
                  )
                  const newPriceInUSNumber = await driverInKeepa.findElements(
                    By2.css('span.priceNew')
                  )
                  if (amazonPriceInUSNumber.length || newPriceInUSNumber.length) {
                    const amazonPriceInUS = amazonPriceInUSNumber.length
                      ? await driverInKeepa.findElement(By2.css('span.priceAmazon')).getText()
                      : await driverInKeepa.findElement(By2.css('span.priceNew')).getText()
                    const newPriceInUS = newPriceInUSNumber.length
                      ? await driverInKeepa.findElement(By2.css('span.priceNew')).getText()
                      : amazonPriceInUS
                    const USTitle =
                      (await driverInKeepa
                        .findElement(By2.css('#productInfoBox > .productTableDescriptionTitle'))
                        .getText()) || ''
                    result.priceInJp = Number(priceInJp)
                    const title = driver[n].findElement(
                      By2.css('.s-result-item.s-asin:nth-child(' + i + ') h2.a-color-base > a')
                    )
